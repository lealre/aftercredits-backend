...
**Status:** overview + Sub-project 1 approved 2026-07-31. LOCAL ONLY — do NOT commit spec or plan (user instruction; `docs/superpowers/` is gitignored anyway).
**Repo:** `github.com/lealre/movies-backend` at `/home/lealre/Github/aftercredits-backend`. Reference for goose/sqlc conventions: the user's private `lealre/chirps-go`.
...
4. **Tooling = goose + sqlc**, chirps-go layout: goose migrations `sql/schema/NNN_*.sql` (`-- +goose Up/Down`), sqlc queries `sql/queries/*.sql` (`-- name: X :one|:many|:exec`, `RETURNING *`, `COALESCE` for partial updates), `sqlc.yaml` v2 engine `postgresql` → generated code in `internal/database`, over a `database/sql` `DBTX` with `WithTx`.
...
4. **Cutover + deploy** — swap wiring to `postgres.Store`; Pi deploy adds a Postgres container + runs goose + drops Mongo/db-setup; backups switch to `pg_dump`; delete Mongo code.
